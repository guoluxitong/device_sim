    ...
    <div  class="app-container">
      ...
      <el-table :data="list.slice((currentPage1-1)*pageSize1,currentPage1*pageSize1)" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row style="width: 120%">
        ...
        <el-table-column align="left" :show-overflow-tooltip="true" label="变更前状态">
          <template slot-scope="scope">
            <span v-if="scope.row.type==1">{{scope.row.rawState | dictionaryValueFieldFilter(lifecycleArray)}}</span>
            <span v-if="scope.row.type==2">{{scope.row.rawState | dictionaryValueFieldFilter(statusArray)}}</span>
            <span v-if="scope.row.type==3">{{scope.row.rawState | dictionaryValueFieldFilter(careStatusArray)}}</span>
          </template>
        </el-table-column>
        <el-table-column align="left" :show-overflow-tooltip="true" label="变更后状态">
          <template slot-scope="scope">
            <span v-if="scope.row.type==1">{{scope.row.nowState | dictionaryValueFieldFilter(lifecycleArray)}}</span>
            <span v-if="scope.row.type==2">{{scope.row.nowState | dictionaryValueFieldFilter(statusArray)}}</span>
            <span v-if="scope.row.type==3">{{scope.row.nowState | dictionaryValueFieldFilter(careStatusArray)}}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column align="left" :show-overflow-tooltip="true" label="变更类型">
          <template slot-scope="scope">
            <span>{{scope.row.type | dictionaryValueFieldFilter(typeArray)}}</span>
          </template>
        </el-table-column>
      </el-table>
      <div>
