  ...
  <div>
    <div  class="app-container">
      ...
      <el-table :data="list.slice((currentPage1-1)*pageSize1,currentPage1*pageSize1)" v-loading="listLoading" element-loading-text="给我一点时间" @row-contextmenu="openTableMenu" border fit highlight-current-row style="width: 120%">
        ...
        <el-table-column align="left" :show-overflow-tooltip="true" label="在线时间">
          <template slot-scope="scope">
            <span>{{dateFormat(scope.row.onlineTime1) }}</span>
          </template>
        </el-table-column>
        <el-table-column align="left" :show-overflow-tooltip="true" label="在线时间">
          <template slot-scope="scope">
            <span>{{dateFormat(scope.row.onlineTime2) }}</span>
          </template>
        </el-table-column>
        <el-table-column align="left" :show-overflow-tooltip="true" label="在线时间">
          <template slot-scope="scope">
            <span>{{dateFormat(scope.row.onlineTime3) }}</span>
          </template>
        </el-table-column>
      </el-table>
...
<script>
  ...
  import {editSim} from '@/api/simcore'
  import {getSim} from '@/api/simrecharge'
